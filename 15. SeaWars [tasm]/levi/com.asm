...
Ser_Ini proc near 
	...
	mov al,0Ch 
	mov ah,35h 
	int 21h ; вз€ть вектор Int 0Ch в es:bx 
	...
	mov dx,3FBh ; регистр управлени€ линией 
	in al,dx 
	...
	out dx,al 
	mov dx,3F9h ; регистр разрешени€ прерываний 
	mov al,1 ; разрешить прерывани€ по приему 
	out dx,al 
	nop ; и чуть-чуть подождать 
	nop 
	mov dx,3FCh ; регистр управлени€ модемом 
...
Ser_Rst proc near 
	...
	push dx 
Wait_Free: 
	mov dx,3FDh ; регистр состо€ни€ линии 
	in al,dx 
...
ret ; длинный переход 
Is_Int: 
	mov al,64h ; послать EOI дл€ IRQ4 
