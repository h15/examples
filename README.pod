...
=item -make for make file with files' state.

=item -check for check files' state.
...
=item hash_file - it's a file, where state's base should consists. Will be created/rewrited on "-make".

=item pathX - path to file or directory, which should be checked.

=back

=head2 2nd promlem. Change SIGINT handler

Print on SIGINT running pid and gid.

Test:
	./who_is_running 1 | ./who_is_running 3 & ./who_is_running 4 | (./who_is_running 7; ./who_is_running 9)

Where param of ./who_is_running is digit.
